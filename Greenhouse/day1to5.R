...
library(agricolae)
data.final
TA2 <- aggregate(list(data.final$dry.weight, data.final$final.length, data.final$fresh.weight,data.final$root.shoot, data.final$nr.leaves, data.final$pr.growth), 
                 by = list(data.final$treatment, data.final$species),
                 FUN = function(x) c(mean = mean(x))
                 )
TA2 <- as.data.frame(as.matrix(TA2))
names(TA2) <- c("treatment", "species", "dry.weight","final.length","fresh.weight","root.shoot","nr.leaves","pr.growth")
